...
export const LightPillar = ({
  ...
  useEffect(() => {
    ...
    const isLowEndDevice = isMobile || (nav.hardwareConcurrency && nav.hardwareConcurrency <= 4);
    ...
    const settings = qualitySettings[effectiveQuality as 'low' | 'medium' | 'high'] || qualitySettings.medium;
  ...
  if (!webGLSupported) {
    return (
      <div className={`light-pillar-fallback ${className}`} style={{ ...style, mixBlendMode }}>
        WebGL not supported
      </div>
    );
  }

  return <div ref={containerRef} className={`light-pillar-container ${className}`} style={{ ...style, mixBlendMode }} />;
};
